...
export class PublisherRestrictionsDecoder implements SpecificDecoder {

  public decode(vector: Vector<PurposeRestriction>): string {

    const intDecoder: IntDecoder = new IntDecoder();
    const boolEnc: BooleanDecoder = new BooleanDecoder();
    const ranges: number[][] = [];
    let range: number[] = [];
    ...
    // Number of entries
    let rangeString = intDecoder.decode(ranges.length, BitLength.rangeEncodingNumEntries);
    ...
    ranges.forEach((range: number[]): void => {
      ...
      rangeString += boolEnc.decode(!single);

      // second is the first (or only) vendorId
      rangeString += intDecoder.decode(range[0], BitLength.vendorId);

      if (!single) {

        rangeString += intDecoder.decode(range[1], BitLength.vendorId);

      }

      const pr: PurposeRestriction = vector.get(range[0]) as PurposeRestriction;

      rangeString += intDecoder.decode(pr.purposeId, BitLength.purposeRestrictionId);
      rangeString += intDecoder.decode(pr.restrictionType, BitLength.purposeRestrictionType);
